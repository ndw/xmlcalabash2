...
class Serialization(override val parent: Option[Artifact]) extends Artifact(parent) {
  ...
  override def validate(): Boolean = {
    ...
    if (parent.isDefined && !parent.get.outputPorts.contains(_port.get)) {
      throw new XmlPipelineException("badport", s"No such port: ${_port.get}")
    }

    if (_standalone.isDefined) {
      if (!List("true","false","omit").contains(_standalone.get)) {
        throw new XmlPipelineException("badstandalone",
