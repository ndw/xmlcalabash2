...
class UrifyWindowsSpec extends AnyFlatSpec with BeforeAndAfter {
  ...
  "//www.acme.com/lib/acme.js " should " resolve against an absolute URI" in {
    // Note that on Windows, this is already an absolute URI and so the base doesn't matter
    val answer = URIUtils.urify("//www.acme.com/lib/acme.js", "https://wiki.acme.com/fr/categories.html")
    assert(answer == URI.create("file://www.acme.com/lib/acme.js"))
  }
  ...
  "c:Users\\ndw " should " throw an exception if resolved against a base URI on another drive" in {
    assertThrows[XProcException] {
      URIUtils.urify("c:Users\\ndw", "D:\\Data\\")
    }
  }
